...
- [x] XP orb + leveling system — XpGem component, XpGemSystem (magnet r=30, collect r=0.5, speed=8), LevelUpSystem (wiki formula 5+(level-1)*10, 2s iframes); per-level stat scaling: 6 BonusPerLevel fields on PlayerStats applied each level (Might, XpMult, Area, Cooldown, ProjectileSpeed, Duration); set per-character in GameSceneBootstrap (Antonio/Lama +1% Might, Imelda +1% XP, Pasqualina/Giovanna +1% ProjSpeed, Arca -1% CD, Porta +1% Area, Poppea +1% Duration)
